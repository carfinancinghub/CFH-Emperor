// ----------------------------------------------------------------------
// File: MyDisputesPage.tsx
// Path: frontend/src/pages/disputes/MyDisputesPage.tsx
...
//   in the `useMyDisputes` hook, keeping the component clean and presentational.
//
// @todos
// - @free:
...
const useMyDisputes = () => {
  ...
  React.useEffect(() => {
    axios.get('/api/disputes/my-disputes', { headers: { Authorization: `Bearer ${token}` } })
...
const MyDisputesPage: React.FC = () => {
  const { disputes, loading } = useMyDisputes();
