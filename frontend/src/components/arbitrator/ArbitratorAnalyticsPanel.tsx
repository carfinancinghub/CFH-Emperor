...
interface ArbitratorAnalyticsPanelProps {
...
const ArbitratorAnalyticsPanel: React.FC<ArbitratorAnalyticsPanelProps> = ({ stats, isPremium }) => {
  try {
    if (!stats) {
      return (
    ...
    // Since `stats` is guaranteed to be defined after the check, TypeScript allows direct access.
    const {
      totalDisputes = 0,
      resolvedCases = 0,
      avgResolutionTime = 0,
