...
Version: 1.1
Description: Jest tests for LiveAuctionStream component with ≥95% coverage,
             including accessibility (jest-axe) and WebSocket mocks.
...
class MockWebSocket {
  ...
  constructor(url: string) {
    ...
    // Simulate connection opening shortly after construction
    setTimeout(() => {
      this.readyState = 1;
...
describe('LiveAuctionStream', () => {
  ...
  beforeEach(() => {
    ...
    // Mock HTMLMediaElement.prototype.play/load to prevent actual video playback errors in tests
    jest
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByLabelText(
          `Video player for live auction ${mockAuctionId}`,
        ),
      ).toBeInTheDocument();
    });
  ...
  it('displays "No live stream available" if streamUrl is missing', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('No live stream available for this auction yet.'),
      ).toBeInTheDocument();
    });
  });
  ...
  it('displays error message if video fails to load', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Failed to load stream: Network error.'),
      ).toBeInTheDocument();

      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('LiveAuctionStream: Video error'),
        expect.any(Object),
      );

      expect(toast.error).toHaveBeenCalledWith(
        expect.stringContaining('Stream error for auction123: Network error.'),
      );
    });
  });
  ...
  it('displays error if autoplay is blocked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert')).toBeInTheDocument();
      expect(
        screen.getByText('Autoplay blocked. Please click to play.'),
      ).toBeInTheDocument();
    });

    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining('Failed to autoplay video'),
      expect.any(Error),
    );
  });
  ...
  it('displays HTTPS warning in production if not secure', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          'Insecure connection detected. Live streaming requires HTTPS.',
        ),
      ).toBeInTheDocument();
    });
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        'Frontend attempting to load LiveAuctionStream over insecure HTTP in production.',
      ),
    );
  ...
  it('should have no accessibility violations for Wow++ tier', async () => {
    const { container } = render(
      <LiveAuctionStream
        ...
        userTier="wowplus"
        streamUrl={mockStreamUrl}
        interactiveOverlaysEnabled={true}
      />,
    );
    const results = await axe(container);
    expect(results).toHaveNoViolations();
